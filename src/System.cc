...
float calculatefl(float imageP, float focalP) {
    return 2 * std::atan((imageP / 2.0) / focalP);
}

void printMat(cv::Mat& matrix) {
    for (int i = 0; i < matrix.rows; ++i) {
        for (int j = 0; j < matrix.cols; ++j) {
            std::cout << matrix.at<float>(i, j) << "\t"; // 修改这里的数据类型根据实际情况
        }
        std::cout << std::endl;
    }
}

void System::ngp_json(const string &filename, cv::Size imagesize)
 {
        ...
        float fl_x = calculatefl(w, fx);
        float fl_y = calculatefl(h, fy);
        ...
        f << "{ " << endl;
        f << "  \"camera_angle_x\": " << fl_x<<","<<endl;
        f << "  \"camera_angle_y\": " << fl_y<<","<<endl;
        ...
        f << "  \"p1\": " << p1<<","<<endl;
        f << "  \"p2\": " << p2<<","<<endl;
        f << "  \"is_fisheye\": " << " false"<<","<<endl;
        f << "  \"cx\": " <<cx<<","<<endl;
        f << "  \"cy\": " <<cy<<","<<endl;
        f << "  \"w\": " << w <<", "<<endl;
        f << "  \"h\": " << h <<", "<<endl;
        f << "  \"aabb_scale\": " <<16<<", "<<endl;
        f << "  \"frames\": ["<<endl;
        ...
        for(size_t i=0; i<vpKFs.size(); i++)
        {
            ...
            // 构造变换矩阵
            cv::Mat T = cv::Mat::eye(4, 4, R.type()); // 创建一个4x4的单位矩阵
            R.copyTo(T(cv::Rect(0, 0, 3, 3)));       // 将旋转矩阵复制到变换矩阵的前3x3子区域
            t.copyTo(T(cv::Rect(3, 0, 1, 3)));       // 将平移向量复制到变换矩阵的第4列
            double sharpness = pKF->sharpness;

            //写入文件
            f<<"    {"<<endl;
            f<<"      \"file_path\":  \"./images/"<<fixed<<setprecision(6)<<pKF->mTimeStamp<<".png\","<<endl;
            f<<"      \"sharpness\": "<<sharpness<<","<<endl;
            f<<"      \"transform_matrix\": ["<<endl;
            for(int i =0; i<= 3; i++)
            {
                f<<"        "<<"["<<endl;
                f<< setprecision(7) <<"          "<<T.at<float>(i, 0)<<","<<endl;
                f<< setprecision(7) <<"          "<<T.at<float>(i, 1)<<","<<endl;
                f<< setprecision(7) <<"          "<<T.at<float>(i, 2)<<","<<endl;
                f<< setprecision(7) <<"          "<<T.at<float>(i, 3)<<endl;
                f<<"        "<<"]";
                if(i <= 2){f<<","<<endl;}
                else { f<<endl;}
            }   
        ...
        f.close();
        cout << endl
             << "NGP format trajectory saved!" << endl;
